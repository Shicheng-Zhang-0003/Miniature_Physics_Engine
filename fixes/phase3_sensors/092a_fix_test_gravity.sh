#!/usr/bin/env bash
# ============================================================
# FIX 092a — Repair: cylinder drop test has no gravity
#   092's test never called mpe_config_init(), so g_cfg.world.gravity
#   was 0 and no body fell. This rewrites the test to init config,
#   print the gravity value, and sanity-check the control sphere
...
grep -q 'MPE_FTC_092a' "$TEST" && { echo "[SKIP] 092a already applied"; exit 0; }

cp "$TEST" "${TEST}.pre_092a"
...
grep -q 'MPE_FTC_092a' "$TEST" || { echo "[FAIL] test not rewritten"; exit 1; }

# Rebuild and run
cd "$SRC"
if ! make test_cylinder_drop > /tmp/cyl_drop_092a.log 2>&1; then
    if ! grep -q '\[info\] gravity' /tmp/cyl_drop_092a.log; then
        tail -15 /tmp/cyl_drop_092a.log
        echo "[FAIL] 092a: test failed to compile (check mpe_config_init name)"
        exit 1
    fi
fi

if ./test_cylinder_drop; then
    echo "[PASS] 092a: cylinder rests on floor"
else
    echo "[INFO] 092a: see diagnostic above — sphere/cylinder behavior now reflects real gravity"
fi
